...
class MoveService:
    def __init__(self):
        ...
        path = rospack.get_path("monitoring")
        path = path + "/adapter_config.yaml"
        print(path)
    ...
    def load_config(self, yaml_path): 
        ...
        move_config = config.get('Param', {}).get('Move', {}) 
        self.cmd_vel_topic = move_config.get('vel', '/cmd_vel') 
        self.odom_topic = move_config.get('odom', '/odom') 
        self.Kp = move_config.get('Kp', 0.5) 
        self.Ki = move_config.get("Ki", 0.1) 

        print(self.cmd_vel_topic)
